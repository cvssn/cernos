...
[prettier, biome].forEach((bin) => {
    if (!fs.existsSync(bin)) {
        throw new Error([`${path.basename(bin)} não encontrado no path: ${bin}`, 'rode `pnpm i` primeiro'].join('\n'));
    }
});
...
const isPrettierTarget = (path) => EXTENSIONS_TO_FORMAT_WITH_PRETTIER.some((ext) => path.endsWith(ext));
...
const walk = (dir) => {
    fs.readdirSync(dir).forEach((entry) => {
        ...
        if (isPrettierTarget(entryPath))
            prettierTargets.push(entryPath);

        if (isBiomeTarget(entryPath))
            biomeTargets.push(entryPath);
    });
};

fs.readdirSync('.').forEach((cur) => {
    if (ROOT_DIRS_TO_SKIP.includes(cur))
        return;

    if (isDir(cur))
        walk(cur);

    if (isPrettierTarget(cur))
        prettierTargets.push(cur);

    if (isBiomeTarget(cur))
        biomeTargets.push(cur);
});
...
execSync([biome, 'format', '--write', `--config-path=${biomeConfig}`, biomeTargets.join(' ')].join(' '));