...
import type {
	...
	DataTable,
	Workflow,
} from "@/cernos-api-client/cernos-api-client.types";
...
/**
 * carrega os arquivos de dados de scenario do fs
 */
export class ScenarioDataFileLoader {
	async loadDataForScenario(scenario: Scenario): Promise<LoadableScenarioData> {
		const workflows = await Promise.all(
			scenario.scenarioData.workflowFiles?.map((workflowFilePath) =>
				this.loadSingleWorkflowFromFile(
					path.join(scenario.scenarioDirPath, workflowFilePath),
				),
			) ?? [],
		);
		...
		const credentials = await Promise.all(
			scenario.scenarioData.credentialFiles?.map((credentialFilePath) =>
				this.loadSingleCredentialFromFile(
					path.join(scenario.scenarioDirPath, credentialFilePath),
				),
			) ?? [],
		);
		...
		const dataTable = scenario.scenarioData.dataTableFile
			? this.loadSingleDataTableFromFile(
					path.join(
						scenario.scenarioDirPath,
						scenario.scenarioData.dataTableFile,
					),
				)
			: null;
	...
	private loadSingleCredentialFromFile(credentialFilePath: string): Credential {
		...
		try {
			...
			throw new Error(
				`falha ao analisar o arquivo de credencial ${credentialFilePath}: ${e.message}`,
			);
		}
	}
	...
	private loadSingleWorkflowFromFile(workflowFilePath: string): Workflow {
		...
		try {
			...
			throw new Error(
				`falha ao analisar o arquivo de workflow ${workflowFilePath}: ${e.message}`,
			);
		}
	}
	...
	private loadSingleDataTableFromFile(dataTableFilePath: string): DataTable {
		...
		try {
			...
			throw new Error(
				`falha ao analisar o arquivo de tabela de dados ${dataTableFilePath}: ${e.message}`,
			);
		}
	}
}
